...
class SchedulerService:
    ...
    async def _job_post_regular(self, owner_id: int = 0, channel_profile_id: int = 0):
        ...
        try:
            ...
            if not topic:
                fallback_topic = (ch_settings.get("topic_raw") or ch_settings.get("title") or "Обзор актуальных событий и полезные советы").strip()
                topic = f"Свободная тема в рамках: {fallback_topic}"
                logger.info("Topic is empty, improvising based on channel profile: %s", topic)
            if not api_key:
                ...
                try:
                    ...
                    for ch_profile in channels:
                        try:
                            ...
                            posts_enabled = (ch_settings.get("posts_enabled") or "0").strip() not in ("0", "false", "no")
                            if not news_enabled or not posts_enabled:
                                continue
                            channel = (ch_settings.get("channel_target") or "").strip()
                            if not channel:
                                continue
