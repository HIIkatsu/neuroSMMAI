...
_VISUAL_SUBJECT_HINTS: dict[str, dict[str, tuple[str, ...] | tuple[str, ...]]] = {
    "finance": {
        "keywords": ("finance", "инвест", "investment", "банк", "bank", "deposit", "депозит", "card", "карта", "cash", "налич", "receipt", "чек", "atm", "банкомат", "account", "statement", "fees", "commission"),
        "nouns": ("bank", "deposit", "card", "atm", "account", "statement", "cash", "fees", "commission", "investment", "risk"),
        "negative": ("hardware", "laptop", "motherboard", "server", "server-room", "meeting", "conference", "office-team"),
    },
    "cars": {
        "keywords": ("car", "cars", "машин", "авто", "lada", "жигул", "brake", "тормоз", "pedal", "педаль", "wheel", "колес", "service", "traffic", "fuel", "dashboard"),
        "nouns": ("car", "lada", "brake", "pedal", "wheel", "service", "traffic", "fuel", "dashboard"),
        "negative": ("storefront", "local business", "office", "meeting", "hardware"),
    },
    "scooter": {
    ...
    "health": {
        "keywords": ("health", "clinic", "клиник", "doctor", "врач", "hospital", "больниц", "waiting room", "пациент", "регистрат", "registry", "талон"),
    ...
    "local_news": {
        "keywords": ("local business", "локальн", "news", "новост", "service update", "community", "city", "media", "trash", "waste", "garbage", "recycling", "container", "truck", "sorting", "мусор", "переработк", "контейнер"),
        "nouns": ("trash", "waste", "recycling", "garbage container", "garbage truck", "sorting"),
        "negative": ("meeting room", "meeting-room", "conference", "hardware", "generic office people", "generic-office-people", "storefront"),
    },
}
...
def _detect_subject_bundle(text: str) -> tuple[list[str], list[str], str]:
    s = (text or "").lower()
    scooter_terms = ("scooter", "самокат", "micromobility", "микромобиль")
    if any(k in s for k in scooter_terms):
        spec = _VISUAL_SUBJECT_HINTS.get("scooter") or {}
        return [str(x) for x in (spec.get("nouns") or ())], [str(x) for x in (spec.get("negative") or ())], "scooter"
