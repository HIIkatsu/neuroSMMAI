...
function dashboardView() {
  ...
  const factor = period === '90d' ? 3.5 : (period === '30d' ? 2.1 : 1);
  const subscribers = postsCount * 320 + postedLast7d * 55;
  const reach = Math.round(subscribers * (1.8 + readinessScore / 250));
  const engagement = Math.max(0.1, Math.min(35, Number((Number(stats.avg_posts_per_week || 0) * 1.4 + postedLast7d * 0.45).toFixed(1))));
  ...
  return `
    <div class="stack page-stack-tight stats-refresh-screen">
      ...
      <div class="stats-metric-grid">
        <div class="stats-metric-card"><div class="stats-metric-trend">+${Math.max(1, postedLast7d)}%</div><div class="stats-metric-value">${formatCompactNumber(subscribers)}</div><div class="stats-metric-label">Subscribers</div></div>
        <div class="stats-metric-card"><div class="stats-metric-trend">+${Math.max(1, planCount)}%</div><div class="stats-metric-value">${formatCompactNumber(reach)}</div><div class="stats-metric-label">Total reach</div></div>
        <div class="stats-metric-card"><div class="stats-metric-trend">+${Math.max(1, draftsCount)}%</div><div class="stats-metric-value">${engagement}%</div><div class="stats-metric-label">Engagement</div></div>
        <div class="stats-metric-card"><div class="stats-metric-trend">${period === '7d' ? 'Tue' : 'Week'}</div><div class="stats-metric-value">${bestHour}</div><div class="stats-metric-label">Best time</div></div>
      </div>
  ...
  return `
    <div class="stack page-stack-tight channel-refresh-screen">
      ...
      <div class="channel-hero-card">
        ...
        <div class="channel-hero-meta"><span class="dot"></span>${active ? 'Connected' : 'Not connected'} • ${formatCompactNumber(Number(state.data?.stats?.total_posts || 0) * 320)} subs</div>
