...
def _title_quality_issues(title: str, family: str) -> list[str]:
    ...
    cheap_patterns = [
        "шок", "сенсац", "вся правда", "без иллюз", "вас обманы", "никто не говорит", "секрет", "правда ли", "о чем молчат", "о чём молчат", "миф",
...
def _positive_title_guardrails(family: str) -> str:
    base = ("АБСОЛЮТНЫЙ ЗАПРЕТ НА КЛИКБЕЙТ. Заголовок должен быть сухим, строгим и по делу. " "ЗАПРЕЩЕНО использовать интриги, вопросы, слова «секрет», «миф», «правда ли», «о чем молчат». Только суть поста.")
    family = (family or "").strip().lower()
    if family in {"massage", "health", "local_business", "beauty", "cars"}:
        extra = "; только сухая польза"
...
- ЗАПРЕТ НА ВЫДУМАННЫЕ ФАКТЫ И УСЛУГИ: КАТЕГОРИЧЕСКИ запрещено выдумывать цены, сроки доставки, условия работы, акции, статистику или вымышленные бренды. Если этих данных нет в исходном запросе — не пиши их вообще. Не придумывай несуществующий коммерческий сервис (доставки, скидки). Опирайся только на здравый смысл и техническую суть.\n- АКТУАЛЬНОСТЬ ВРЕМЕНИ: КРИТИЧЕСКИ ВАЖНО! НИКОГДА не используй слова «вчера», «сегодня», «недавно», «на днях», оценивая события. В твоей базе данных могут быть новости годовой давности. Описывай события просто как факты без привязки к недавнему времени (например, вместо «Вчера компания выпустила» пиши «Компания выпустила»).
...
- image_prompt: ENGLISH STOCK PHOTO SEARCH QUERY. Write a 1-3 word ENGLISH search query for Pexels/Pixabay. ONLY NOUNS. NO sentences. NO backgrounds. NO adjectives unless absolutely necessary. Examples: "smart tv", "car battery", "massage therapist", "financial chart". This MUST be English.
{extra_rules}
""".strip()
    ...
    *,
    ...
    async def _run_once(user_prompt: str, temperature: float = 0.82) -> dict[str, str]:
        raw = await ai_chat(
            ...
            temperature=0.25, # Жесткая фиксация от галлюцинаций
            base_url=base_url,
            max_tokens=400,
        )
    ...
    for idx, angle in enumerate(angle_pool, start=1):
        ...
        normalized = await _run_once(prompt_text, temperature=0.25)
