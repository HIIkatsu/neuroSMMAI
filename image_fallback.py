...
async def search_stock_candidates(query: str, *, query_family: str = "primary") -> list[StockCandidate]:
    ...
    if PEXELS_API_KEY:
        for item in await _search_pexels(query, limit=3):
            candidates.append(
                StockCandidate(
                    url=item.get("url", ""),
                    provider="pexels",
    ...
    if PIXABAY_API_KEY:
        for item in await _search_pixabay(query, limit=3):
            candidates.append(
                StockCandidate(
                    url=item.get("url", ""),
                    provider="pixabay",
...
async def _search_pexels(query: str, *, limit: int = 1) -> list[dict[str, str]]:
...
async def _search_pixabay(query: str, *, limit: int = 1) -> list[dict[str, str]]:
